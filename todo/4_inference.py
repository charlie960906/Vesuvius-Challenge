...
segment_path = '.\\data'
segment_id = 'casey_mask'
model_path = '.vesuvius_challenge/todo/model/timesformer_wild15_20230702185753_0_fr_i3depoch=12.ckpt'
...
def read_image_mask(segment_id, start_idx, end_idx, rotation=0, part='part_0_0'):
  image_stack = []
  mid = 65 // 2

  # mask processing
  fragment_mask = cv2.imread(f"C:\\Users\\charl\\Desktop\\science\\layers\\{part}\\20230702185753_mask.png", 0)
  print(os.path.join(segment_path, segment_id, f"{segment_id}_mask.png"))
  ...
  for i in range(start_idx, end_idx):
    image = cv2.imread(f"C:\\Users\\charl\\Desktop\\science\\layers\\{part}\\{i:02}.tif", 0)
...
def get_img_splits(segment_id, start_idx, end_idx, rotation=0, part='part_0_0'):
  image_stack, fragment_mask, image_shape, (xs, ys, xe, ye) = read_image_mask(segment_id, start_idx, end_idx, part=part)
...
def predict_fn(loader, model, image_shape, part):
  ...
  predict_folder = f".\\vesuvius_challenge\\todo\\predict\\{segment_id}\\"
  ...
  filename = f".\\vesuvius_challenge\\todo\\predict\\{segment_id}\\{segment_id}_{part}.png"
...
if __name__ == "__main__":
  ...
  parts = [f'part_{i}_{j}' for i in range(13) for j in range(11)]

  for part in parts:
    loader, coords, image_shape, fragment_mask = get_img_splits(segment_id, start_idx, end_idx, part=part)
    if (loader == None): continue

    model = RegressionPLModel.load_from_checkpoint(r"C:\Users\charl\Desktop\science\vesuvius_challenge\todo\model\timesformer_wild15_20230702185753_0_fr_i3depoch=12.ckpt", map_location=device, strict=False)
    model.eval()

    predict_fn(loader, model, image_shape, part)
